-- Ploščice zdaj sledijo pevčevemu glasu namesto splošnemu ritmu, zato sta se
-- spremenila število not in največji možni rezultat na komad. Vrednosti so
-- strežniško zrcalo `maxScore` in `notes.length` iz `src/data/game.ts`; brez
-- tega popravka strežnik zavrne vsak nov rezultat kot 'invalid score'.
-- Vrednosti izpiše `scripts/build-charts.py` ob generiranju kart.
...
BEGIN
  ...
  SELECT CASE p_song_id
    WHEN 'mrfy' THEN 107500
    WHEN 'kokosy' THEN 136700
    WHEN 'tabu' THEN 153500
  END,
  CASE p_song_id
    WHEN 'mrfy' THEN 283
