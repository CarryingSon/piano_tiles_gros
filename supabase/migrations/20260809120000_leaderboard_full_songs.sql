-- Igra se je z 36-sekundnega izseka premaknila na cel komad, zato so se
-- spremenili največji možni rezultat in število not na komad. Vrednosti so
-- strežniško zrcalo `maxScore` in `notes.length` iz `src/data/game.ts` — ob
-- vsaki novi beat mapi (`scripts/build-charts.py`) je treba posodobiti oboje.
--
-- Poleg tega je najkrajša veljavna seja skrajšana s 30 na 10 sekund: igra se
-- zdaj lahko povsem legitimno konča že po nekaj sekundah, ko igralec porabi
-- vseh sedem življenj, in tak rezultat mora biti mogoče oddati.
...
BEGIN
  ...
  SELECT CASE p_song_id
    WHEN 'mrfy' THEN 173500
    WHEN 'kokosy' THEN 113500
    WHEN 'tabu' THEN 234700
  END,
  CASE p_song_id
    WHEN 'mrfy' THEN 448
    WHEN 'kokosy' THEN 298
    WHEN 'tabu' THEN 601
  END
